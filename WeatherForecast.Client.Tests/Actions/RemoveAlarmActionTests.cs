...
{
    public class RemoveAlarmActionTests
    {
        ...
        [Fact]
        public async Task RemoveAlarmActionTests_Success()
        {
            var expected = Core.Domain.Models.Enums.MessageType.AlarmOff;
            var got = await new RemoveAlarmAction(CreateMockService()).Do(
                new Message()
                {
